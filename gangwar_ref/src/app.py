...
def load_current_drug_prices():
    ...
    try:
        ...
        # Create default base prices even if config file is missing or corrupted
        base_prices = {
            'weed': 500,
            'crack': 1000,
            'coke': 2000,
...
def update_daily_drug_prices():
    """Update drug prices for new day with wild fluctuations"""
    import time
...
# Global high scores list - persists for app runtime
HIGH_SCORES_LIST = []
...
# Comment 1: HIGH_SCORES_FILE variable definition removed since we're using in-memory storage
...
def load_high_scores() -> List[HighScore]:
    """Load high scores from memory (resets on app restart)"""
    global HIGH_SCORES_LIST
    if not HIGH_SCORES_LIST:
        # Initialize with default scores if empty
        HIGH_SCORES_LIST = generate_default_high_scores()
    return HIGH_SCORES_LIST[:]

def save_high_scores(scores: List[HighScore]):
    """Save high scores to memory (resets on app restart)"""
    global HIGH_SCORES_LIST
    HIGH_SCORES_LIST = scores[:] + []  # Copy the list to avoid reference issues
...
def calculate_score(money_earned: int, days_survived: int, gang_wars_won: int, fights_won: int) -> int:
    ...
    # Money earned contributes 1 point per $10000 (further reduced to slow down early game)
    money_score = money_earned // 10000

    # Days survived contributes 10 points per day (further reduced)
    survival_score = days_survived * 10

    # Gang war victories contribute 100 points each (reduced)
    gang_war_score = gang_wars_won * 100

    # Individual fights won contribute 5 points each (reduced)
    fight_score = fights_won * 5
