...
class GenericLocationScreen extends StatelessWidget {
                    ...
                    children: [
                      Text(
                        ...
                        style: TextStyle(
                          ...
                          color: _getThemeColor(locationType).withValues(alpha: 0.2),
                        ),
                      ),
  ...
  String _getLocationDescription(String type) {
    return switch (type) {
      'hospital' => 'The sterile smell of antiseptic barely masks the stench of death. Doctors here don\'t ask questions if you have the cash.',
      'police' => 'The precinct is a fortress of state power. Every shadow here feels like a pair of handcuffs.',
      'store' => 'A dusty corner shop selling overpriced essentials to those who can\'t leave the neighborhood.',
      'gym' => 'The clang of iron and the scent of sweat. A good place to toughen up your crew.',
      'warehouse' => 'Rats and rusted crates. The perfect place for a quiet deal or a shallow grave.',
      'hideout' => 'A fortified safehouse where you can lie low when the heat gets too high.',
      'house' => 'A decaying tenement building. The walls have ears and the doors have three locks.',
      _ => 'A nondescript building in the heart of the urban sprawl.',
    };
  }

  Widget _buildActionButtons(BuildContext context, GameProvider gameProvider, String type) {
    switch (type) {
      case 'hospital':
        return Column(
          children: [
              ...
              onPressed: () {
                if (gameProvider.gameState.spendMoney(2000)) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Doctors patched you up. Health restored!')),
                  );
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
            ...
            ElevatedButton(
              onPressed: () {
                if (gameProvider.gameState.spendMoney(5000)) {
                   gameProvider.gameState.reputation = (gameProvider.gameState.reputation * 0.5).toInt();
                   gameProvider.gameMessage = 'You bribed the precinct. Heat reduced!';
                   ScaffoldMessenger.of(context).showSnackBar(
                     const SnackBar(content: Text('Bribe accepted. The cops are looking the other way... for now.')),
                   );
                }
              },
            ...
            ElevatedButton(
              onPressed: () {
                if (gameProvider.gameState.spendMoney(1000)) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Training complete. Max health increased!')),
                  );
                }
              },
      ...
      default:
        return const Text('No special actions available here yet.', style: TextStyle(color: Colors.white60));
    }
  }
}
